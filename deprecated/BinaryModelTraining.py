...
right_droop_train_labels = [fn.split("\\")[-1].split("_")[2].strip() for fn in right_droop_train]
...
left_droop_train_labels = [fn.split("\\")[-1].split("_")[2].strip() for fn in left_droop_train]
...
right_droop_validate_labels = [fn.split("\\")[-1].split("_")[2].strip() for fn in right_droop_validate]
...
left_droop_validate_labels = [fn.split("\\")[-1].split("_")[2].strip() for fn in left_droop_validate]
...
right_droop_test_labels = [fn.split("\\")[-1].split("_")[2].strip() for fn in right_droop_test]
...
left_droop_test_labels = [fn.split("\\")[-1].split("_")[2].strip() for fn in left_droop_test]
...
test_generator = test_datagen.flow(test_imgs, test_labels_enc, batch_size=BATCH_SIZE_VALIDATION)
...
class MyModel(tf.keras.Model):
  ...
  def __init__(self):
    ...
    self.max1  = tf.keras.layers.MaxPooling2D(2)
